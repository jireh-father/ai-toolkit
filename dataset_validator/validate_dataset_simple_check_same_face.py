"""Same-face hairstyle identity validator (INPUT vs OUTPUT).

Compares input and output images of the same person to determine if
the hairstyle is completely identical (i.e. no change occurred).
Returns true/false per sample with a reason.
...
- INPUT = the original image of a person before any editing.
- OUTPUT = the result image of the same person after editing.

Both images show the SAME person. The face, body, and background may differ slightly due to the editing process. Focus ONLY on the hairstyle.

TASK: Determine whether the hairstyle in OUTPUT is COMPLETELY IDENTICAL to the hairstyle in INPUT. In other words, check if the hairstyle was NOT changed at all during editing.
...
Only answer true if the hairstyle is completely identical between INPUT and OUTPUT (i.e. no hairstyle change occurred).
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Same-Face Hairstyle Identity Validator (INPUT vs OUTPUT)",
...
# ---------------------------------------------------------------------------
# Dataset scanning (2-dir: input + output)
# ---------------------------------------------------------------------------
...
def _build_messages_2img_qwen(images, prompt):
    return [{
        "role": "user",
        "content": [
            {"type": "text", "text": "[INPUT] Original image:"},
            {"type": "image", "image": images[0]},
            {"type": "text", "text": "[OUTPUT] Edited result:"},
...
<title>Same-Face Hairstyle Identity Report</title>
...
<body>
<div class="container">
  <h1>Same-Face Hairstyle Identity Report</h1>
  <div class="meta">
  ...
  <div class="controls">
    ...
    <div>
      ...
      <select id="sortSelect" onchange="applyFilter()">
        <option value="name-asc">Filename (A &rarr; Z)</option>
        <option value="name-desc">Filename (Z &rarr; A)</option>
...
def main():
    ...
    logger = logging.getLogger(__name__)
    logger.info("=" * 60)
    logger.info("Same-Face Hairstyle Identity Validator (INPUT vs OUTPUT)")
    logger.info("=" * 60)
