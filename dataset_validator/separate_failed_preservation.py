"""Separate preserved and not-preserved samples based on preservation validation results.

Works with results.json produced by validate_dataset_preservation.py,
which returns match: true/false per sample.

Usage:
    python dataset_validator/separate_failed_preservation.py \
        --report ./reports_preservation/results.json \
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Separate preserved/not-preserved samples based on preservation results",
    )
    parser.add_argument(
        "--report", type=str, required=True,
        help="Path to results.json from validate_dataset_preservation.py",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Destination folder for not-preserved samples",
    )
    parser.add_argument(
        "--passed-dir", type=str, default=None,
        help="Destination folder for preserved samples",
    )
    parser.add_argument(
...
def main():
    ...
    passed_files = []
    for r in results:
        if r.get("error") or r.get("match") is None:
    ...
    logger.info(f"Preserved: {len(passed_files)}, Not preserved: {len(failed_files)}")
    ...
    # Process failed (not preserved)
    if args.failed_dir and failed_files:
        failed_dir = Path(args.failed_dir)
        logger.info(f"{op_name} {len(failed_files)} not-preserved samples to {failed_dir}...")
        success, skip = _copy_files(
            failed_files, input_dir, reference_dir, output_dir, failed_dir, op_func, op_name,
        )
        logger.info(f"  Not preserved — processed: {success}, skipped: {skip}")

    # Process passed (preserved)
    if args.passed_dir and passed_files:
        passed_dir = Path(args.passed_dir)
        logger.info(f"{op_name} {len(passed_files)} preserved samples to {passed_dir}...")
        success, skip = _copy_files(
            passed_files, input_dir, reference_dir, output_dir, passed_dir, op_func, op_name,
        )
        logger.info(f"  Preserved — processed: {success}, skipped: {skip}")

    logger.info("=" * 50)
    logger.info("Done!")
    if args.failed_dir:
        logger.info(f"  Not preserved dir: {Path(args.failed_dir).resolve()}")
    if args.passed_dir:
        logger.info(f"  Preserved dir: {Path(args.passed_dir).resolve()}")
    logger.info("=" * 50)
