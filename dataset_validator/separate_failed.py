"""Separate passed and failed samples from the dataset based on validation results.
...
def is_pass_with_threshold(scores: dict, threshold: int) -> bool:
    """Check if all score fields meet the threshold."""
    if not scores:
        return False
    return all(scores.get(f, 0) >= threshold for f in SCORE_FIELDS)


def parse_args():
    parser = argparse.ArgumentParser(
        description="Separate passed/failed samples from dataset based on validation report",
    )
    parser.add_argument(
        "--report", type=str, required=True,
        help="Path to results.json from validate_dataset.py",
    )
    parser.add_argument(
...
def main():
    ...
    for r in results:
        ...
        if r.get("error"):
            ...
            continue
        if args.threshold is not None:
            if is_pass_with_threshold(scores, threshold):
